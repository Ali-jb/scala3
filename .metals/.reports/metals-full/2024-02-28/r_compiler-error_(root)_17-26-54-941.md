file://<WORKSPACE>/src/main/scala/Module%2012/ex.worksheet.sc
...
action parameters:
uri: file://<WORKSPACE>/src/main/scala/Module%2012/ex.worksheet.sc
text:
```scala
...
object Account {
  ...
  def applyTransaction(transaction: Transaction) =
    transaction match {
      ...
      case Withdrawal(Savings, accountNum, amount) if getAccount(Savings, accountNum).balance < amount =>
        throw new IllegalStateException

      case Withdrawal(Savings, accountNum, amount) =>        
        getAccount(Savings, accountNum).withdrawal(amount)

      case Withdrawal(Checking, accountNum, amount) if (getAccount(Checking, accountNum).balance + 1000) < amount =>
        throw new IllegalStateException
...
class Module12Solutions extends FunSpec with Matchers with StopOnFirstFailure with SeveredStackTraces {
...
scala.runtime.Scala3RunTime$.assertFailed(Scala3RunTime.scala:11)
	...
	dotty.tools.dotc.core.Types$OrType.widenUnionWithoutNull(Types.scala:3450)
