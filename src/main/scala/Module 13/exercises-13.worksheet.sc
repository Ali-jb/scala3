...
def combineZippedSetsAndString(s1: List[Set[Char]], seq: String): List[Set[Char]] = 
    val l = stringToChars(seq).zip(s1)
    l.map{list => list(1) + list(0)}
...
//     (numbers.head, numbers.filter(_ < numbers.head), numbers.filter(_>= numbers.head))
...
def sort(numbers: List[Int]): List[Int] = 
      ...
      case l : List[Int] => {
          val head :: rest = l
          val (lower, remaining) = rest.partition(_ < head)
          sort(lower) ::: List(head) ::: sort(remaining)
      }
    }

// test("Can you tell what it is yet") {
val newList = sort(numbers)
