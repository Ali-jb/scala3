...
val comboListWithIndex = allCombos.zipWithIndex
...
val filteredListWithIndex = comboListWithIndex.filter{(set, size) => set.size > 1}.map(_.swap)
 filteredListWithIndex.toString
...
val mutationMap: Map[Int, Set[Char]] =  filteredListWithIndex.toMap
Map.empty ++ filteredListWithIndex
...
object KeyValueCollection {
  def sortByKey(keyValueMap: Map[String, Int]): List[(String, Int)] = 
    val sortedMap = ListMap(keyValueMap.toSeq.sortBy(_._1): _*)
    sortedMap.toList
  

  def sortByValue(keyValueMap: Map[String, Int]): List[(String, Int)] = 
    val sortedMap = ListMap(keyValueMap.toSeq.sortBy(_._2): _*)
    sortedMap.toList
}
 val unsortedMap = Map("b" -> 2, "a" -> 1, "c" -> 3)
 
 KeyValueCollection.sortByKey(unsortedMap)
 KeyValueCollection.sortByValue(unsortedMap)
